...
void MovieSearch::loadActorFile(string file_name)
{
	...
	if (in_stream.is_open() == true)
	{
		...
		while (in_stream.good() == true)
		{
			...
			string actor_id = data[ID_INDEX]; 
			string actor_name = data[NAME_INDEX]; 
			vector<string> movie_ids = 
				StringSplitter::split(data[MOVIES_INDEX], ","); 
	...
	unordered_map<Actor*, int>& visited_actors)
{
		...
		if (visited_movies[movie] < 1)
		{
			...
			for (auto actor : movie->getActors())
			{
				...
				if (visited_actors[actor] < 1)
				{
					...
					// if we hit a dead end, keep going until we hit the deadest of ends
					// (i.e. we get to the top of the call stack and we still haven't found
					// a connection
					if (found == false)
					{
						continue;
	...
	// if we get to a dead end, then return false
	return false;
}
	...
	unordered_map<Movie*, Actor*>& pre_movies)
{
	...
	while(current_actor != start)
	{
		...
		path.push(pre_actors[current_actor]->getId()); 
...
void MovieSearch::task3(string actor_id)
{
	...
	{
		cout << actor_movies[i]->getId() << ", ";
	}
	cout << actor_movies[actor_movies.size() - 1]->getId() << endl;
